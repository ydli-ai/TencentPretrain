...
class GenerateLm(torch.nn.Module):
    ...
    def forward(self, src, tgt, seg):
        ...
        output = self.target.lm.output_layer(memory_bank)

        next_token = torch.multinomial(F.softmax(output[0][-1], dim=-1), num_samples=1)
        tgt = torch.cat([tgt, next_token.view(1, 1)], dim=1)
        loss = self.target.lm(memory_bank, tgt, seg)
        print(loss)
